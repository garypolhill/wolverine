extensions [ gis table csv math mgr ]
...
to-report update-memory [ unhappiness-list ]
  if length unhappiness-list = 0 or first unhappiness-list != ticks [
    report fput ticks unhappiness-list
  ]
  report unhappiness-list
end
...
to association-offer-houses
  set move-waiting-list filter [ hh -> [not household-in-intervention?] of hh ] move-waiting-list
  repeat moves-per-tick [
    if length move-waiting-list > 0 [
      ...
      if not find-apartment-for-hh? request-hh options-per-move [
        set move-waiting-list lput request-hh move-waiting-list
      ]
    ]
  ]
end
...
to households-accumulate-encounters
  ...
  if (here-n-encounters > 0                                                  ; IF there's at least 1 other person here ...
    ...
    ask one-of households-here with [household-ethnicity != my-ethnicity] [  ; THEN pick someone to give some kind of racism-related unhappiness to
      set unhappiness-racism update-memory unhappiness-racism
      if networked-unhappiness? [
...
to households-update-crime
  if household-criminal? and random-float 1 < p-crime [
    ...
    ask victim [
      set unhappiness-crime update-memory unhappiness-crime
...
### Changes following SSC 2022 feedback

- Call this wolverine 'v2' rather than 'lt'
- Drop the use of the lt extension (it makes things worse!)
- Unhappinesses only add the tick if it isn't there already
